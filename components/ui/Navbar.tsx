...
export default function Navbar({ items = defaultItems }: NavbarProps) {
  ...
  return (
    <div className="w-full px-6 pt-5 sticky top-0 z-50">
      <nav
        className="mx-auto flex w-full max-w-[1248px] items-center gap-6 px-6 py-3 backdrop-blur-[16.5px]"
        style={{
          borderRadius: 16,
      ...
      >
        ...
        <ul className="hidden md:flex items-center gap-7 list-none m-0 p-0">
          {items.map((item) => (
            <li key={item.href}>
              <Link
                href={item.href}
                className="text-base font-normal whitespace-nowrap transition-colors hover:text-white px-8 py-2"
        ...
        {/* Desktop search pill */}
        <div className="relative hidden md:block shrink-0 ml-auto" style={{ width: 160, height: 32 }}>
          <input
            type="text"
            placeholder=""
            aria-label="Search"
      ...
      {menuOpen && (
        <div
          className="mx-auto mt-2 w-full max-w-[760px] px-6 pb-5 pt-4 flex flex-col gap-4 backdrop-blur-[16.5px]"
          style={{
            borderRadius: 24,
            border: "1px solid rgba(255,255,255,0.13)",
            background: "rgba(255,255,255,0.03)",
          ...
          <ul className="flex flex-col gap-3 list-none m-0 p-0">
            {items.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="block text-base font-normal transition-colors hover:text-white"
                  style={{ color: "#D8D8D8", fontFamily: "Inter, sans-serif" }}
