...
export default function DbTestPage() {
  ...
  async function handleCheckTime() {
    ...
    try {
      ...
      if (error) {
        ...
        setDbTime(`Connection Successful! DB Time: ${new Date(data).toLocaleString()}`);
      }
    } catch (err) {
  ...
  <main className="flex min-h-screen flex-col items-center justify-center p-24 bg-slate-900 text-white">
    <div className="z-10 w-full max-w-md items-center justify-between font-mono text-sm lg:flex flex-col gap-6 bg-slate-800 p-8 rounded-xl shadow-2xl border border-slate-700">
      <h1 className="text-2xl font-bold tracking-tight">PUP ACCESS-WEB DB Test</h1>
      ...
      {dbTime && (
        <div className={`mt-4 p-4 rounded-md w-full text-center ${
          dbTime.includes('Error') ? 'bg-red-900/30 text-red-400 border border-red-800' : 'bg-green-900/30 text-green-400 border border-green-800'
        }`}>
          <p className="text-xs uppercase tracking-widest font-bold mb-1">Result</p>
